...
const Orders = () => {
  ...
  const handleStatus = async (id, status) => {
    try {
      ...
      setAlert({ show: true, type: 'success', title: t('success'), message: 'Order status updated successfully', onConfirm: null });
      fetchOrders();
    } catch(e) {
      const errorMsg = e.response?.data?.message || e.response?.data?.error || e.message || 'Failed to update status';
    ...
    setAlert({
      ...
      onConfirm: async () => {
        try {
          ...
          setAlert({ show: true, type: 'success', title: t('success'), message: 'Order cancelled successfully', onConfirm: null });
          setSelected(null);
          fetchOrders();
        } catch(e) {
  ...
  const handlePrint = (order) => {
    ...
    const content = `
      ...
      <body>
        ...
        <div class="item-row"><strong>Order #:</strong> ${order.id}</div>
        <div class="item-row"><strong>Time:</strong> ${formatDate(order.created_at)}</div>
        <div class="item-row"><strong>Type:</strong> ${translateType(order.type)}</div>
        <div class="divider"></div>
        <strong>Items:</strong>
        ${order.items?.map(it => `
          <div class="item-row">
        ...
        <div class="item-row"><span>Tax (${appSettings.tax_rate || 20}%):</span> <span>$${tax.toFixed(2)}</span></div>
        <div class="item-row total"><span>TOTAL:</span> <span>$${total.toFixed(2)}</span></div>
        <div class="divider"></div>
            ...
            <div className="bg-zinc-800/30 rounded-lg p-3 space-y-2">
              {(() => {
                ...
                return (
                  <>
                    <div className="flex items-center justify-between">
                      <p className="text-[12px] text-zinc-500">Subtotal:</p>
                      <p className="text-[12px] font-semibold text-zinc-300">${itemsSubtotal.toFixed(2)}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-[12px] text-zinc-500">Tax ({appSettings.tax_rate || 20}%):</p>
