    ...
    <div className="space-y-5 max-w-[1400px]">
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title={t('addProduct')}>
        <form onSubmit={handleCreate} className="space-y-3">
          <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Nom</label><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="ex: Pizza Margherita" className="input-dark" /></div>
          <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Description</label><input value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Optionnel" className="input-dark" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Prix ($)</label><input required type="number" step="0.01" value={form.price} onChange={e=>setForm({...form,price:e.target.value})} placeholder="0.00" className="input-dark" /></div>
            <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Catégorie</label><select required value={form.category_id} onChange={e=>setForm({...form,category_id:e.target.value})} className="input-dark"><option value="">Sélectionner...</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>
      ...
      <Modal isOpen={showModify} onClose={() => setShowModify(false)} title={t('modifyProduct')}>
        <form onSubmit={handleModify} className="space-y-3">
          ...
          <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Nom</label><input required value={modifyForm.name} onChange={e=>setModifyForm({...modifyForm,name:e.target.value})} className="input-dark" /></div>
          <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Description</label><input value={modifyForm.description} onChange={e=>setModifyForm({...modifyForm,description:e.target.value})} className="input-dark" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Prix ($)</label><input required type="number" step="0.01" value={modifyForm.price} onChange={e=>setModifyForm({...modifyForm,price:e.target.value})} className="input-dark" /></div>
            <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Catégorie</label><select required value={modifyForm.category_id} onChange={e=>setModifyForm({...modifyForm,category_id:e.target.value})} className="input-dark"><option value="">Sélectionner...</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>

          <div>
            <label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Disponibilité</label>
            ...
            <div className="space-y-2">
              ...
              {modifyIngredients.map((row, idx) => (
                <div key={idx} className="grid grid-cols-[1fr_110px_40px] gap-2 items-center">
                  ...
                  <button type="button" onClick={() => removeModifyIngredientRow(idx)} className="w-10 h-10 rounded-xl flex items-center justify-center text-zinc-400 hover:text-red-400 hover:bg-red-500/10" title="Supprimer">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              ))}
      ...
      <Modal isOpen={showCat} onClose={() => setShowCat(false)} title={t('addCategory')}>
        <form onSubmit={handleCat} className="space-y-3">
          <div><label className="block text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-1">Nom</label><input required value={catName} onChange={e=>setCatName(e.target.value)} placeholder="ex: Boissons, Desserts" className="input-dark" /></div>
