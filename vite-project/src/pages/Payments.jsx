...
import { CreditCard, DollarSign, RefreshCw, CheckCircle, Banknote } from 'lucide-react';
...
const Payments = () => {
  ...
  const handlePaymentClick = (orderId, method) => {
    ...
    setAlert({
      ...
      message: `Do you want to confirm payment for order #${orderId} for ${formatMoney(totalWithTax)}?`,
      onConfirm: () => {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[13px]">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {payments.length === 0 ? (
                      <tr><td colSpan="6" className="text-center py-12 text-zinc-600">{t('noPayments')}</td></tr>
                    ) : payments.map(p => (
                      <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-5 py-3.5">
                          <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-lg text-emerald-400 bg-emerald-500/10">Payé</span>
                        </td>
