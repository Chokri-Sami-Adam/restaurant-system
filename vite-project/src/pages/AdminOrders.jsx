...
const AdminOrders = () => {
  ...
  const handleCreateOrder = async () => {
    if (formItems.some(item => !item.product_id || item.quantity < 1)) {
      setAlert({ show: true, type: 'error', title: t('error'), message: 'Please select products and quantities', onConfirm: null });
      return;
    }
    ...
    try {
      ...
      setAlert({ show: true, type: 'success', title: t('success'), message: 'Order created successfully', onConfirm: null });
  ...
  const handleEditOrder = async () => {
    ...
    try {
      ...
      setAlert({ show: true, type: 'success', title: t('success'), message: 'Order updated successfully', onConfirm: null });
      setEditModal({ show: false, order: null });
      setFormItems([{ product_id: '', quantity: 1 }]);
      setOrderType('dine-in');
      fetchData();
    } catch (e) {
      const errorMsg = e.response?.data?.message || 'Failed to update order';
      setAlert({ show: true, type: 'error', title: t('error'), message: errorMsg, onConfirm: null });
    }
  };
    ...
    setAlert({
      ...
      onConfirm: async () => {
        try {
          ...
          setAlert({ show: true, type: 'success', title: t('success'), message: 'Order deleted successfully', onConfirm: null });
          fetchData();
        } catch (e) {
          const errorMsg = e.response?.data?.message || e.message || 'Failed to delete order';
          setAlert({ show: true, type: 'error', title: t('error'), message: errorMsg, onConfirm: null });
        }
      }
    });
    ...
    <div className="space-y-5 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Create and manage orders</p>
        </div>
      ...
      <div className="flex gap-1 bg-[#18181b]/50 backdrop-blur-sm border border-white/5 rounded-xl p-1 w-fit">
        {['all', 'pending', 'preparing', 'ready', 'served', 'cancelled'].map(status => (
          <button key={status} onClick={() => setFilter(status)} className={`text-[12px] font-semibold px-4 py-2 rounded-lg transition-all ${
            ...
            {status.charAt(0).toUpperCase() + status.slice(1)} {status === 'all' && <span className="text-amber-400 ml-1">({orders.length})</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          <p className="text-[15px] font-bold text-zinc-300">No orders found</p>
        </div>
      ) : (
      ...
      {/* Create Order Modal */}
      <Modal isOpen={createModal.show} onClose={() => setCreateModal({ show: false })} title="Create Order">
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-zinc-400 uppercase">Order Type</label>
          ...
          <div>
            <label className="text-xs font-semibold text-zinc-400 uppercase mb-2 block">Items</label>
          ...
          <div className="border-t border-white/10 pt-4 text-right">
            <p className="text-sm text-zinc-500 mb-3">Total: <span className="text-amber-400 font-bold text-lg">{formatMoney(calculateTotal())}</span></p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setCreateModal({ show: false })} className="btn-ghost">Cancel</button>
      ...
      {/* Edit Order Modal */}
      <Modal isOpen={editModal.show} onClose={() => setEditModal({ show: false, order: null })} title={`Edit Order #${editModal.order?.id}`}>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-zinc-400 uppercase">Order Type</label>
            <select value={orderType} onChange={(e) => setOrderType(e.target.value)}
          ...
          <div className="border-t border-white/10 pt-4 text-right">
            <p className="text-sm text-zinc-500 mb-3">Total: <span className="text-amber-400 font-bold text-lg">{formatMoney(calculateTotal())}</span></p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setEditModal({ show: false, order: null })} className="btn-ghost">Cancel</button>
              <button onClick={handleEditOrder} className="btn-primary">Update Order</button>
            </div>
          </div>
        </div>
      </Modal>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] text-zinc-600 uppercase font-semibold tracking-wider">Type</p>
                <p className="text-[13px] font-semibold text-white capitalize mt-1">{selected.type || 'N/A'}</p>
              </div>
              <div>
                <p className="text-[10px] text-zinc-600 uppercase font-semibold tracking-wider">Total</p>
                <p className="text-[13px] font-semibold text-amber-400 mt-1">{formatMoney(calculateTotalWithTax(selected.total_price))}</p>
              </div>
              <div>
                <p className="text-[10px] text-zinc-600 uppercase font-semibold tracking-wider">Status</p>
                <p className={`text-[11px] font-bold uppercase px-2 py-1 rounded inline-block mt-1 ${sc(selected.status)}`}>{selected.status}</p>
              </div>
              <div>
                <p className="text-[10px] text-zinc-600 uppercase font-semibold tracking-wider">Date</p>
                <p className="text-[13px] font-semibold text-white mt-1">{new Date(selected.created_at).toLocaleString()}</p>
              </div>
            </div>

            <div className="border-t border-white/5 pt-4">
              <p className="text-[10px] text-zinc-600 uppercase font-semibold tracking-wider mb-2">Items</p>
